...
object ImplicitConversions {
  ...
  implicit def toCallback[T](fc: FutureCallback[T]): Callback[T] = FutureCallbackCallback(fc)
  ...
  private case class FutureCallbackCallback[T](fc: FutureCallback[T]) extends Callback[T] {
    override def onFailure(call: Call[T], t: Throwable): Unit = fc.failed(new Exception(t))

    override def onResponse(call: Call[T], response: Response[T]): Unit =
      if (response.isSuccessful)
        fc.completed(response.body())
      else
        fc.failed(new Exception(response.errorBody().string()))
  }
